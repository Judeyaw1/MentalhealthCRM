...
async function main() {
  ...
  for (const record of records) {
    if (
      !record.therapistId ||
      !validUserIds.has(record.therapistId.toString())
    ) {
  ...
  if (badRecords.length === 0) {
    ...
    console.log(
      `Found ${badRecords.length} treatment records with invalid therapist references.`,
    );
    for (const record of badRecords) {
      console.log(
        `- Record ID: ${record._id}, therapistId: ${record.therapistId}`,
      );
